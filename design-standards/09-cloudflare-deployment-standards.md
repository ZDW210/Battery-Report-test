...
# 2026-06-16 EIOT R2 归档补充

- Worker 的 R2 绑定名保持 `BUCKET`，线上 bucket 使用 `baobiao-eiot-archive`。
- EIOT 原始报文统一写入 `eiot/meter/` 或 `eiot/alarm/` 前缀，文件名包含推送时间戳、网关编号和 UUID，避免同一网关同一秒批量推送时覆盖。
- 管理端文件上传和 EIOT 原始报文归档共用当前 Worker 的 `BUCKET` 绑定；后续如要拆分业务附件桶和 EIOT 归档桶，必须新增绑定名并同步更新 Worker 代码和部署文档。
