...
const filterProjectRoutes = (routes: AppRouteRecordRaw[]): AppRouteRecordRaw[] => {
  return routes.filter(keepProjectRoute).map((route) => {
    const path = route.path.startsWith('/') ? route.path : `/${route.path}`
    if (path !== '/energy' || !route.children?.length) {
      return route
    }
    ...
    return {
      ...
      children: route.children.filter((child) => {
        ...
        const menuKey = segments[0] === 'energy' ? segments[1] : segments[0]
        return ENERGY_MENU_CHILDREN.has(menuKey)
      })
    }
  })
  ...
  actions: {
    async generateRoutes(): Promise<unknown> {
      return new Promise<void>(async (resolve) => {
        ...
        const routerMap: AppRouteRecordRaw[] = filterProjectRoutes(generateRoute(res))
        // 动态路由，404一定要放到最后面
